...
export default function App() {
  const cards = [
    ...
    },
    {
      ...
      name: "Switch",
      supertype: "Trainer",
      rules: ["Switch 1 of your own Benched Pokémon with your Active Pokémon."],
      number: "95",
      artist: "Keiji Kinebuchi",
      rarity: "Common",
      legalities: {
  ...
  return (
    <>
      <h1>PTC Seller</h1>
      {
        // villkor ? true : om false
        cart.length == 0 ? (
          <p>Cart is empty</p>
        ) : (
          cart.map((item, index) => <p key={item + "-" + index}>{item}</p>)
        )
      }

      {cards.map((card, index) => (
        <Card index={index} card={card} addToCart={addToCart} />
      ))}
    </>
  );
}
